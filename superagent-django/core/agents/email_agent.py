...
Summarize emails AND send the summary to someone:
  1. read_emails  → returns {"emails": [...], "count": N}
  2. summarize_emails(emails=result["emails"])  → returns formatted_summary
  3. send_email(to=<recipient>, subject="Email Summary", body=<formatted_summary>)
  CRITICAL: The body of send_email MUST be the FULL formatted_summary text returned by
  summarize_emails — never use a placeholder like "Here is a summary" without the content.
...
CRITICAL — If read_emails returns 0 emails or an empty list:
  → You MUST immediately call search_emails with query="in:inbox" and max_results=10
...
- Use a generic placeholder like "Here is a summary of your latest emails:" as an email body —
  always pass the FULL formatted_summary from summarize_emails as the body
