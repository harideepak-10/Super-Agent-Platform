...
class AnthropicProvider(LLMProvider):
    ...
    def __init__(self, model: str = _MODEL, max_tokens: int = _MAX_TOKENS) -> None:
        ...
        Args:
            model: Claude model name to use (default: claude-sonnet-5).
    ...
    def _translate_tools(tools: list[dict[str, Any]] | None) -> list[dict[str, Any]] | None:
        """Convert OpenAI/Anthropic-style function-calling tool schemas to Anthropic's format.
    ...
    def _parse_response(self, completion: Any) -> dict[str, Any]:
        ...
        for block in completion.content:
            if block.type == "text":
                ...
                # Only the first tool call is surfaced — same contract as AnthropicProvider.
                tool_call = {
                    "name": block.name,
